...
public class SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        // TODO: Fix me
        Set<String> separate1 = new HashSet<>();
        Set<String> separate2 = new HashSet<>();
        for (String label : g.labels()) {
            if (!separate1.contains(label) && !separate2.contains(label)) {
                separate1.add(label);
            }
            for (String enemy : g.neighbors(label)) {
                if (separate1.contains(label) && separate1.contains(enemy) || separate2.contains(label) && separate2.contains(enemy)) {
                    return false;
                }
                if (separate1.contains(label) && !separate2.contains(enemy)) {
                    separate2.add(enemy);
                } else if (separate2.contains(label)) {
                    separate1.add(enemy);
                }
            }
        }
