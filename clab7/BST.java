...
public class BST<Key extends Comparable<Key>> {
    ...
    public int traversal(Node root, int len) {
        int path;

        if (root == null) return 0;

        path = len;
        path += traversal(root.left, len + 1);
        path += traversal(root.right, len + 1);

        return path;
    }

    public double optimalAverageDepth() {
        return (double) traversal(root, 0) / size();
    }
